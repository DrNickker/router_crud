...
function NewPost() {
  const addNewPostSubmitHandler = async (event) => {
    ...
    if (formValue.length > 0) {
      ...
      console.log("url", `${process.env.REACT_APP_POSTS_URL}/posts`)
      await axios.post(`${process.env.REACT_APP_POSTS_URL}/posts`, data).then((responce) => console.info("req: ", responce)).catch((error) => console.log(error));
    }
